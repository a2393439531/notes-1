...
其他直接通过HTTP操作进行修改（使用HTTP PUT替换整个图，或者HTTP DELETE删除一个RDF图）或者复取（通过HTTP GET）RDF图的操作在[SPARQL 1.1 图存储HTTP协议](http://www.w3.org/TR/sparql11-http-rdf-update/)中有详细描述。这个规范可被看成是SPARQL 1.1 Query和Update的轻量级合成。
